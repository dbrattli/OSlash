...
class TestListFunctor(unittest.TestCase):
    ...
    def test_list_functor_law2(self):
        ...
        # Empty list
        y = List.empty()
        self.assertEqual(y.map(compose(f, g)), y.map(g).map(f))

        # Long list
        z = List.from_iterable(range(42))
        self.assertEqual(z.map(compose(f, g)), z.map(g).map(f))
...
class TestListApplicative(unittest.TestCase):
    def test_list_applicative_law_functor(self):
        ...
        # Empty list
        z = empty()
        self.assertEqual(pure(f).apply(z), z.map(f))
    ...
    def test_list_applicative_law_identity(self):
        ...
        # Singleton list
        x = unit(42)
        self.assertEqual(pure(identity).apply(x), x)

        # Empty list
        y = List.empty()
        self.assertEqual(pure(identity).apply(y), y)
    ...
    def test_list_applicative_binary_func_singleton(self):
        ...
        self.assertEqual(pure(f).apply(v).apply(w), unit(42))

    def test_list_applicative_unary_func(self):
        f = lambda x: x * 2
        v = unit(21)

        self.assertEqual(pure(f).apply(v), unit(42))
    ...
    def test_list_applicative_binary_func(self):
        ...
        self.assertEqual(pure(f).apply(v).apply(w), List.from_iterable([5, 9, 6, 10]))

    def test_list_applicative_empty_func(self):
        v = unit(42)
        w = List.from_iterable([1, 2, 3])

        self.assertEqual(empty().apply(v).apply(w), empty())
    ...
    def test_list_applicative_binary_func_empty_arg_1(self):
        ...
        self.assertEqual(pure(f).apply(e).apply(v), empty())
    ...
    def test_list_applicative_binary_func_empty_arg_2(self):
        ...
        self.assertEqual(pure(f).apply(v).apply(e), empty())
...
class TestListMonad(unittest.TestCase):
    ...
    def test_list_monad_empty_bind(self):
        ...
        self.assertEqual(m.bind(f), empty())
    ...
    def test_list_monad_law_left_identity(self):
        ...
        self.assertEqual(unit(v).bind(f), f(v))
